...
public class Ball {
    ...
    public void moveBallX(int dir, Rectangle[] bound) {
        for (int i = c; i >= 0; i--) {
            ...
            for (int j = 0; j < bound.length; j++)
                if (Rectangle.isIntersectRectBall(bound[j], new Ball(x + dir * i, y)))
                    ok = 0;
            if (ok == 1) {
                x += dir * i;
                return;
    ...
    public void moveBallY(int dir, Rectangle[] bound) {
        for (int i = c; i >= 0; i--) {
            int ok = 1;
            for (int j = 0; j < bound.length; j++)
                if (Rectangle.isIntersectRectBall(bound[j], new Ball(x, y + dir * i)))
                    ok = 0;
            if (ok == 1) {
                y += dir * i;
                return;
    ...
    public int isBounded(Ball b, Rectangle[] bound) {
        int ok = 1;
        for (int j = 0; j < bound.length; j++)
            if (Rectangle.isIntersectRectBall(bound[j], b))
                ok = 0;
        return ok;
    }
